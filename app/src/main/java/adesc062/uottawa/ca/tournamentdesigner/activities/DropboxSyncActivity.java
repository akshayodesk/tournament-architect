    ...
    public class GetUsernameTask extends AsyncTask<Void, Boolean, Boolean> {
        protected Boolean doInBackground(Void... params) {
            ...
            if (syncIsActivated && accessToken != null) {
                try {
                    DbxRequestConfig requestConfig = new DbxRequestConfig("TournamentDesigner/1.1");
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            if (syncIsActivated && accessToken != null) {
                try {
                    DbxRequestConfig requestConfig = new DbxRequestConfig("TournamentDesigner/1.1");
                    sDbxClient = new DbxClientV2(requestConfig, accessToken);
                    final String localFilePath = "/data/data/adesc062.uottawa.ca.tournamentdesigner/databases/TournamentDesignerDatabase.s3db";
                    File localFile = new File(localFilePath);
                    InputStream inputStream = new FileInputStream(localFile);
                    sDbxClient.files().delete("/TournamentDesignerDatabase.s3db");
                    sDbxClient.files().uploadBuilder("/TournamentDesignerDatabase.s3db").uploadAndFinish(inputStream);
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            if (syncIsActivated && accessToken != null) {
                try {
                    DbxRequestConfig requestConfig = new DbxRequestConfig("TournamentDesigner/1.1");
                    sDbxClient = new DbxClientV2(requestConfig, accessToken);
                    DbxDownloader<FileMetadata> dbxDownloader = sDbxClient.files().download("/TournamentDesignerDatabase.s3db");
                    InputStream inputStream = dbxDownloader.getInputStream();
                    OutputStream outputStream = new FileOutputStream("/data/data/adesc062.uottawa.ca.tournamentdesigner/databases/TournamentDesignerDatabase.s3db");
